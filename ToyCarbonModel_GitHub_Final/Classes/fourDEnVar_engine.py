...
class fourDEnVar_engine:
    ...
    def __init__(self, Xb, hX, y, R, hxbar):   
        ...
        if len(np.shape(self.Xb))==1:
            self.n=1
            self.nens=np.shape(self.Xb)[0]
            self.xbar = np.mean(Xb)
        else:
            self.n=np.shape(self.Xb)[0]
            self.nens=np.shape(self.Xb)[1]
            self.xbar = np.mean(self.Xb,1)
    ...
    def fourDEnVar(self):
        ...
        minimiser = spop.minimize(self.fourDEnVar_cost_f, np.zeros((self.nens,1)), method='L-BFGS-B', jac=self.fourDEnVar_cost_df,                                          options={'gtol': 1e-16,'maxiter':1000000})
        ...
        work2=np.dot(self.Yb_dash.T,np.linalg.pinv(work1))
        w_analytical=-np.dot(work2,self.hxbar-self.y)
        #np.save('ideal_w.npy',w_analytical)
    ...
    def fourDEnVar_sample_posterior(self):
        ...
        Wa = np.linalg.cholesky(work3) # temporary
