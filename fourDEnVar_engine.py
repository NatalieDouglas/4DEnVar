...
class fourDEnVar_engine:
    ...
    def __init__(self, xb, hx, y, R, hx_bar):   
        ...
        self.HXb_dash = self.scale*(hx-hx_bar.reshape(self.nobs,1))
        print('shape of HX_dash_b: ' + str(np.shape(self.HXb_dash)))
    ...
    def fourDEnVar(self):
        ...
        # CHECK
        work1=np.dot(self.HXb_dash.T,np.dot(self.R_inv,self.HXb_dash))
        work2=np.dot(self.HXb_dash.T,np.dot(self.R_inv,self.hx_bar-self.y))
    ...
    def fourDEnVar_cost_f(self, w):
        ...
        work1=np.dot(self.HXb_dash,w)+self.hx_bar-self.y
    ...
    def fourDEnVar_cost_df(self, w):
        ...
        df=np.dot(self.HXb_dash.T,work2)
    ...
    def fourDEnVar_sample_posterior(self):
        ...
        work1 = np.dot(self.R_inv,self.HXb_dash)
        work2 = np.dot(self.HXb_dash.T,work1)+np.identity(self.nens)
        if np.allclose(work2,work2.T) and np.all(np.linalg.eigvals(work2) > 0):
            print('Xbdash*inv(I+HXbdash^T*Rinv*HXbdash)*Xbdash^T is symmetric and all eigenvalues are positive')
